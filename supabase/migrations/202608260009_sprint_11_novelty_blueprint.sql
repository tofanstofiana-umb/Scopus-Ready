-- Sprint 11: activate Novelty Builder and Article Blueprint and extend the
-- validated structured worksheet persistence boundary to both modules.
update public.worksheet_modules
set is_active = true
where code in ('novelty', 'blueprint');
...
begin
  ...
  if target_module_code not in ('literature', 'gap', 'novelty', 'blueprint') then
