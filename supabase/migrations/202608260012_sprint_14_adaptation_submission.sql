-- Sprint 14: activate Journal Adaptation and a deterministic Submission
-- Checklist. Both persist through worksheet_answers; checklist completion is
-- derived exclusively from five boolean confirmations.
update public.worksheet_modules
set is_active = true
where code in ('journal_adaptation', 'submission');
...
begin
  ...
  if target_module_code not in (
    ...
    'scientific_story', 'internal_review', 'journal_adaptation', 'submission'
  ) then
  ...
  expected_keys := case target_module_code
    ...
    when 'journal_adaptation' then array['author_guidelines', 'title_abstract_keywords', 'structure_word_limit', 'citations_references', 'submission_package']::text[]
    when 'submission' then array['manuscript_file_ready', 'journal_format_confirmed', 'metadata_complete', 'ethics_and_declarations_complete', 'supplementary_files_ready']::text[]
  end;
  ...
  foreach field_key in array expected_keys loop
    if target_module_code = 'submission' then
      if jsonb_typeof(target_content -> field_key) is distinct from 'boolean' then
        raise exception 'INVALID_CONTENT' using errcode = '22023';
      end if;
      if (target_content ->> field_key)::boolean then
      ...
      max_length := case
        ...
        when target_module_code = 'journal_adaptation' and field_key = 'submission_package' then 1500
        else 2000
      end;
  ...
  calculated_status := case
    ...
    when target_module_code = 'submission' and calculated_completion = 100 then 'completed'
    else 'in_progress'
  end;
